...
`tools/list` at `https://gateway.pipeworx.io/arcgis-yakima/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/arcgis_yakima_search_datasets \
  -H 'Content-Type: application/json' \
  -d '{"query":"parcels"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/arcgis_yakima_search_datasets`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "arcgis-yakima": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-arcgis-yakima"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-arcgis-yakima
```
